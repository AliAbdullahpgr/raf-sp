...
export default function AdminDashboardViewsPage() {
  ...
  const sorted = Object.entries(data.adminDeptViewMap).sort(([, a], [, b]) => b - a);
  const maxViews = Math.max(...Object.values(data.adminDeptViewMap), 1);
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">Views on /dashboard/[dept] pages - logged-in users only</p>
          </div>
      ...
      <Card className="border-slate-200 shadow-sm">
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {sorted.length === 0 && <div className="py-8 text-center text-slate-400">No admin dashboard views yet</div>}
          </div>
        </CardContent>
      </Card>
    </div>
